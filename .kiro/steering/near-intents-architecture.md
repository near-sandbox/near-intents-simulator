...
console.log('🔄 [INTENTS] Requesting quote:', { from, to, amount });

// Success logs
console.log('✅ [INTENTS] Quote generated:', { quoteId, amountOut, timeEstimate });
...
### Log Categories
- `🔄 [INTENTS]` - Processing/calculation steps
- `✅ [INTENTS]` - Successful operations
- `❌ [INTENTS]` - Errors and failures
- `⚙️  [INTENTS]` - Execution details
- `🔗 [INTENTS]` - Cross-chain operations
...
ethereum:native          → ETH
ethereum:usdc.eth        → USDC on Ethereum
bitcoin:native           → BTC
```
