...
// ❌ Bad: Implicit any or null handling
...
// ❌ Bad: Inheritance with overrides
interface Quote {
  amountIn: string;
  amountOut: string;
  fee: string;
...
```typescript
// ✅ Good: Proper mock implementations
...
// Error logs
console.error('❌ [INTENTS] Quote calculation failed:', {
  error: error.message,
  request: request,
  stack: error.stack
});
